...
class oua_advanced_testcase extends advanced_testcase {
     ...
     */
    public function assertValidHtml($htmlstring) {
        $decodedhtmlstring = strtr($htmlstring, $this->HTML401NamedToNumeric); // Remove html named entities;
        if (strpos(strtolower($htmlstring), '<html') === false) { // Wrap if not a full html document.
            $wrappedstring = "<wrapper>$decodedhtmlstring</wrapper>";
            $wrappederrormsg = "Fragment has been auto wrapped";
        } else {
            $wrappedstring = $decodedhtmlstring;
            $wrappederrormsg = "";
        }

        $doc = new DOMDocument('1.0', 'utf-8');
        libxml_use_internal_errors(true); // Allow us to process errors rather than just sending them to me.
        libxml_clear_errors(); // Clear errors from previous run.

        $domloaded = $doc->loadXML($wrappedstring, LIBXML_NOCDATA | LIBXML_NOWARNING | LIBXML_NOERROR);
        $errors = libxml_get_errors();
        $error_message = $this->ConstructErrorMessage("HTML Fragment was not Formed correctly XML Parser Errors found ($wrappederrormsg):", $wrappedstring, $errors);
        $this->AssertTrue($domloaded, $error_message);
        // Ensure we are error free in the template.
        $this->assertCount(0, $errors, $error_message);
    }
    ...
    private function ConstructErrorMessage($title, $contents, $errors) {
        $error_message = $title . var_export($errors, true) . "\n\n\nDocument was:\n\n" . $this->FormatLineNumbers($contents,
                                                                                                                   $errors);
    ...
    /**
     * Find the count of a particular node in the document by XPath query string
     * @param $expectedlength how many nodes you expect to find
     * @param $query the query to find the nodes
     * @param $html the html/xml to find the node in
     * @param $errormsg error message if expected nodes are not found
     */
    public function assertXpathDomQueryResultLengthEquals($expectedlength, $query, $html, $errormsg) {
        $doc = new DOMDocument();
        $doc->loadHTML($html);
        $selector = new DOMXPath($doc);
        $result = $selector->query($query);
        $this->assertEquals($expectedlength, $result->length, $errormsg);
    }

    /**
     * Find all nodes with specific classes (class="myclass")
     * compares to an array of values of these nodes.
     *
     * @param $expectedresult array of values of expected nodes.
     * @param $searchclass nodes with class to search
     * @param $html html/xml to search
     * @param $errormsg
     */
    public function assertXPathGetNodesWithClassesEquals($expectedresult, $searchclass, $html, $errormsg){
        $doc = new DOMDocument();
        $doc->loadHTML($html);
        $selector = new DOMXPath($doc);
        $result = $selector->query("//*[contains(concat(' ', normalize-space(@class), ' ') , ' $searchclass ')]");
        foreach ($result as $node) {
            $values[] = $node->nodeValue;
        }
        $this->assertEquals($expectedresult, $values, $errormsg);
    }
    /**
     * Find all nodes with id's containing specified id (id="*myid*")
     * compares to an array of values of these nodes.
     *
     * @param $expectedresult array of values of expected nodes.
     * @param $searchclass nodes with id to search
     * @param $html html/xml to search
     * @param $errormsg
     */
    public function assertXPathGetNodesWithIdsEquals($expectedresult, $searchid, $html, $errormsg){
        $doc = new DOMDocument();
        $doc->loadHTML($html);
        $selector = new DOMXPath($doc);
        $result = $selector->query("//*[contains(@id, '$searchid')]");
        foreach ($result as $node) {
            $values[] = $node->nodeValue;
        }
        $this->assertEquals($expectedresult, $values, $errormsg);
    ...
    /**
     * Find all nodes with specific classes (class="myclass")
     * ensures the count is correct
     *
     * @param $expectedresult int number of expected nodes.
     * @param $searchclass nodes with class to search
     * @param $html html/xml to search
     * @param $errormsg
     */
    public function assertXPathGetNodesWithClassesCount($expectedresult, $searchclass, $html, $errormsg){
        $doc = new DOMDocument();
        $doc->loadHTML($html);
        $selector = new DOMXPath($doc);
        $result = $selector->query("//*[contains(concat(' ', normalize-space(@class), ' ') , ' $searchclass ')]");
        $this->assertEquals($expectedresult, $result->length, $errormsg);
    }
}
